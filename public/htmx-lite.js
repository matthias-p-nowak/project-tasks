 ...
 * @param {*} event 
 */
hxl_submit_form = function (event) {
 ...
 */
hxl_send_form = async function (action, formData, target) {
    try {
 ...
 * @param {*} responseText 
 */
hxl_process_body = function (responseText) {
    ...
    for (const n of div.querySelectorAll("[x-action]")) {
        ...
        switch (attr) {
            ...
            case 'error':
                {
                    const se = document.getElementById(showErrorId);
                    se.innerHTML = '';
                    se.append(n);
                    se.show();
                }
